...
export function Sidebar() {
  ...
  return (
    <motion.aside
      ...
      className="fixed left-0 top-[var(--app-titlebar-height)] h-[calc(100vh-var(--app-titlebar-height))] bg-ink-950 text-bone-50 border-r border-ink-800 flex flex-col z-40 overflow-hidden"
    >
      {/* Logo / Brand */}
      <div className={cn(
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          {!isCollapsed && (
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-[10px] text-ink-400 mt-0.5 uppercase tracking-wider">Portal</span>
            </div>
          )}
        </div>
        {!isCollapsed && (
              ...
              return (
                <div key={section.label} className="pb-1">
                  <button
                    ...
                    className="flex w-full items-center justify-between rounded-sm px-3 py-1.5 text-[10px] font-medium uppercase tracking-wider text-ink-500 transition-colors duration-hover ease-brand hover:text-ink-300"
                  >
